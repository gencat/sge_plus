  ...
  module SignatureCollection
    module Admin
      describe CandidacyForm do
        ...
        let(:other_scope) { create(:candidacies_type_scope, type: candidacies_type) }
        let(:state) { "open" }
        ...
        let(:type_id) { candidacies_type.id }
        ...
        describe "#map_model" do
          ...
          context "when there are offline votes" do
            ...
            context "with the global scope" do
              let(:scope) { create(:candidacies_type_scope, scope: nil) }
              ...
              context "when there are votes" do
                before { candidacy.update!(offline_votes: { "total" => "100", "global" => "100" }) }
        ...
        describe "#scoped_type_id" do
          context "when created from attributes" do
            ...
            context "when type_id and decidim_scope_id from candidacy are provided" do
              it { is_expected.to eq(candidacy.scoped_type.id) }
            end
          ...
          context "when created from model" do
            subject { described_class.from_model(candidacy).with_context(context).scoped_type_id }
