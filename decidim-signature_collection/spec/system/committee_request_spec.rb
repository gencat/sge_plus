  ...
  context "when GET new" do
    context "and owner requests membership" do
      it "Owner is not allowed to request membership" do
        ...
        create(:authorization, user: candidacy.author)
        login_as candidacy.author, scope: :user

        visit decidim_candidacies.new_candidacy_committee_request_path(candidacy.to_param)
    ...
    context "and authorized user" do
      ...
      it "are allowed to request membership" do
        ...
        visit decidim_candidacies.new_candidacy_committee_request_path(candidacy.to_param)
        expect(page).to have_content("You are about to request becoming a member of the promoter committee of this candidacy")
      end
    end
    ...
    context "and user is not connected" do
      ...
      it "are allowed to request membership" do
        expect(page).to have_current_path decidim_candidacies.new_candidacy_committee_request_path(candidacy.to_param)
        expect(page).to have_content("You are about to request becoming a member of the promoter committee of this candidacy")
      end
