# frozen_string_literal: true

require "spec_helper"

describe "Filter Candidacies", :slow do
  let!(:organization) { create(:organization) }
  let!(:type1) { create(:candidacies_type, organization:) }
  let!(:type2) { create(:candidacies_type, organization:) }
  let!(:type3) { create(:candidacies_type, organization:) }
  let!(:scoped_type1) { create(:candidacies_type_scope, type: type1) }
  let!(:scoped_type2) { create(:candidacies_type_scope, type: type2) }
  let!(:scoped_type3) { create(:candidacies_type_scope, type: type3, scope: nil) }
  let!(:area_type1) { create(:area_type, organization:) }
  let!(:area_type2) { create(:area_type, organization:) }
  let!(:area1) { create(:area, area_type: area_type1, organization:) }
  let!(:area2) { create(:area, area_type: area_type1, organization:) }
  let!(:area3) { create(:area, area_type: area_type2, organization:) }

  before do
    switch_to_host(organization.host)
  end

  context "when filtering candidacies by SCOPE" do
    let!(:candidacies) { create_list(:candidacy, 2, organization:, scoped_type: scoped_type1) }
    let(:first_candidacy) { candidacies.first }
    let!(:proposal_comment) { create(:comment, commentable: first_candidacy) }

    before do
      create(:candidacy, organization:, scoped_type: scoped_type2)
      create(:candidacy, organization:, scoped_type: scoped_type3)

      visit decidim_candidacies.candidacies_path
    end

    it "can be filtered by scope" do
      within "form.new_filter" do
        expect(page).to have_content(/Scope/i)
      end
    end

    context "when selecting all scopes" do
      it "lists all candidacies", :slow do
        within "#panel-dropdown-menu-scope" do
          click_filter_item "All"
        end

        expect(page).to have_css(".card__grid", count: 4)
        expect(page).to have_content("4 candidacies")
      end
    end

    context "when selecting the global scope" do
      it "lists the filtered candidacies", :slow do
        within "#panel-dropdown-menu-scope" do
          click_filter_item "Global"
        end

        expect(page).to have_css(".card__grid", count: 1)
        expect(page).to have_content("1 candidacy")
      end
    end

    context "when selecting one scope" do
      it "lists the filtered candidacies", :slow do
        within "#panel-dropdown-menu-scope" do
          click_filter_item scoped_type1.scope_name[I18n.locale.to_s]
        end

        expect(page).to have_css(".card__grid", count: 2)
        expect(page).to have_content("2 candidacies")
      end

      it "can be ordered by most commented after filtering", skip: "Awaiting review" do
        within "#panel-dropdown-menu-scope" do
          click_filter_item scoped_type1.scope_name[I18n.locale.to_s]
        end

        within "#dropdown-menu-order" do
          click_on "Most commented"
        end

        expect(page).to have_css(".card__grid[id^='candidacy']", count: 2)
        expect(page).to have_css(".card__grid[id^='candidacy']:first-child", text: translated(first_candidacy.title))
      end
    end
  end

  context "when filtering candidacies by STATE" do
    before do
      create_list(:candidacy, 4, organization:)
      create_list(:candidacy, 3, :accepted, organization:)
      create_list(:candidacy, 2, :rejected, organization:)
      create(:candidacy, :acceptable, organization:)
      create(:candidacy, organization:, answered_at: Time.current)

      visit decidim_candidacies.candidacies_path
    end

    it "can be filtered by state" do
      within "form.new_filter" do
        expect(page).to have_content(/Status/i)
      end
    end

    context "when selecting all states" do
      it "lists all candidacies", :slow do
        stub_const("Decidim::Paginable::OPTIONS", [100])
        within "#panel-dropdown-menu-state" do
          click_filter_item "All"
          click_filter_item "All"
        end

        expect(page).to have_css(".card__grid", count: 11)
        expect(page).to have_content("11 candidacies")
      end
    end

    context "when selecting the open state", skip: "Awaiting review" do
      it "lists the open candidacies", :slow do
        within "#panel-dropdown-menu-state" do
          click_filter_item "All"
          click_filter_item "Open"
        end

        expect(page).to have_css(".card__grid", count: 5)
        expect(page).to have_content("5 candidacies")
      end
    end

    context "when selecting the closed state" do
      it "lists the closed candidacies" do
        within "#panel-dropdown-menu-state" do
          click_filter_item "Open"
          click_filter_item "Closed"
        end

        expect(page).to have_css(".card__grid", count: 6)
        expect(page).to have_content("6 candidacies")
      end
    end

    context "when selecting the accepted state" do
      it "lists the accepted candidacies" do
        within "#panel-dropdown-menu-state" do
          click_filter_item "Open"
          click_on(id: "dropdown-trigger-with_any_state_state_closed")
          click_filter_item "Enough signatures"
        end

        expect(page).to have_css(".card__grid", count: 3)
        expect(page).to have_content("3 candidacies")
      end
    end

    context "when selecting the rejected state" do
      it "lists the rejected candidacies" do
        within "#panel-dropdown-menu-state" do
          click_filter_item "Open"
          click_on(id: "dropdown-trigger-with_any_state_state_closed")
          click_filter_item "Not enough signatures"
        end

        expect(page).to have_css(".card__grid", count: 2)
        expect(page).to have_content("2 candidacies")
      end
    end

    context "when selecting the answered state" do
      it "lists the answered candidacies" do
        within "#panel-dropdown-menu-state" do
          click_filter_item "Open"
          click_filter_item "Answered"
        end

        expect(page).to have_css(".card__grid", count: 1)
        expect(page).to have_content("1 candidacy")
      end
    end
  end

  context "when filtering candidacies by TYPE" do
    context "when there is a single candidacy_type" do
      let(:type2) { nil }
      let(:type3) { nil }
      let(:scoped_type2) { nil }
      let(:scoped_type3) { nil }

      before do
        create_list(:candidacy, 3, organization:, scoped_type: scoped_type1)

        visit decidim_candidacies.candidacies_path
      end

      it "does not display TYPE filter" do
        expect(page).to have_no_css("#panel-dropdown-menu-type")
      end

      it "lists all candidacies", :slow do
        expect(page).to have_css(".card__grid", count: 3)
        expect(page).to have_content("3 candidacies")
      end
    end

    context "when there is more than one candidacy_type" do
      before do
        create_list(:candidacy, 2, organization:, scoped_type: scoped_type1)
        create(:candidacy, organization:, scoped_type: scoped_type2)

        visit decidim_candidacies.candidacies_path
      end

      it "can be filtered by type" do
        within "form.new_filter" do
          expect(page).to have_content(/Type/i)
        end
      end

      context "when selecting all types" do
        it "lists all candidacies", :slow do
          within "#panel-dropdown-menu-type" do
            click_filter_item "All"
          end

          expect(page).to have_css(".card__grid", count: 3)
          expect(page).to have_content("3 candidacies")
        end
      end

      context "when selecting one type" do
        it "lists the filtered candidacies", :slow do
          within "#panel-dropdown-menu-type" do
            click_filter_item type1.title[I18n.locale.to_s]
          end

          expect(page).to have_css(".card__grid", count: 2)
          expect(page).to have_content("2 candidacies")
        end
      end
    end
  end

  context "when filtering candidacies by AREA" do
    before do
      create_list(:candidacy, 2, organization:, area: area1)
      create(:candidacy, organization:, area: area2)
      create(:candidacy, organization:, area: area3)

      visit decidim_candidacies.candidacies_path
    end

    it "can be filtered by area" do
      within "form.new_filter" do
        expect(page).to have_content(/Area/i)
      end
    end

    context "when selecting all areas" do
      it "lists all candidacies", :slow do
        within "#panel-dropdown-menu-area" do
          click_filter_item "All"
          click_filter_item "All"
        end

        expect(page).to have_css(".card__grid", count: 4)
        expect(page).to have_content("4 candidacies")
      end
    end

    context "when selecting one area" do
      it "lists the filtered candidacies", :slow do
        within "#panel-dropdown-menu-area" do
          within ".filter", text: area_type1.name[I18n.locale.to_s] do
            find("button[aria-expanded='false']").click
          end
          within ".filter", text: area_type2.name[I18n.locale.to_s] do
            find("button[aria-expanded='false']").click
          end
          click_filter_item area1.name[I18n.locale.to_s]
        end

        expect(page).to have_css(".card__grid", count: 2)
        expect(page).to have_content("2 candidacies")
      end
    end

    context "when selecting one area type" do
      it "lists the filtered candidacies", :slow do
        within "#panel-dropdown-menu-area" do
          click_filter_item area_type1.name[I18n.locale.to_s]
        end

        expect(page).to have_css(".card__grid", count: 3)
        expect(page).to have_content("3 candidacies")
      end
    end
  end

  context "when filtering candidacies by AUTHOR" do
    context "when not logged in" do
      before do
        visit decidim_candidacies.candidacies_path
      end

      it "cannot be filtered by author" do
        within "form.new_filter" do
          expect(page).to have_no_content(/Author/i)
        end
      end
    end

    context "when logged in" do
      let(:user) { create(:user, :confirmed, organization:) }

      before do
        create_list(:candidacy, 2, organization:, author: user)
        create_list(:candidacy, 1, :created, organization:, author: user)
        create(:candidacy, organization:)

        login_as user, scope: :user

        visit decidim_candidacies.candidacies_path
      end

      it "can be filtered by author" do
        within "form.new_filter" do
          expect(page).to have_content(/Author/i)
        end
      end

      context "when selecting any author" do
        it "lists all candidacies", :slow do
          within "#panel-dropdown-menu-author" do
            click_filter_item "Any"
          end

          expect(page).to have_css(".card__grid", count: 3)
          expect(page).to have_content("3 candidacies")
        end
      end

      context "when selecting my candidacies" do
        it "lists the filtered candidacies", :slow do
          within "#panel-dropdown-menu-author" do
            click_filter_item "My candidacies"
          end

          expect(page).to have_css(".card__grid", count: 3)
          expect(page).to have_content("3 candidacies")
        end
      end
    end
  end
end
