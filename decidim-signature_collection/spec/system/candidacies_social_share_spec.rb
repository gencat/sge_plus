...
describe "Social shares" do
  ...
  let!(:candidacy) { create(:candidacy, description:, scoped_type: candidacy_type_scope, organization:) }
  let!(:candidacy_type) { create(:candidacies_type, banner_image:, organization:) }
  let!(:candidacy_type_scope) { create(:candidacies_type_scope, type: candidacy_type) }
  let!(:attachment) { create(:attachment, :with_image, attached_to: candidacy, file: attachment_file) }
  ...
  context "when listing all assemblies" do
    let(:resource) { decidim_candidacies.candidacies_path }
