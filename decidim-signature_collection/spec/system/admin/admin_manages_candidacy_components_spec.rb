...
describe "Admin manages candidacy components", skip: "Awaiting review" do
  let(:organization) { create(:organization) }
  let(:user) { create(:user, :admin, :confirmed, organization:) }

  let!(:candidacy) { create(:candidacy, organization:) }
  let!(:attributes) { attributes_for(:component, participatory_space: candidacy) }
  ...
  context "when adds a component" do
    before do
      visit decidim_admin_candidacies.components_path(candidacy)
    ...
    it "has a successful admin log" do
      ...
      expect(page).to have_content("created #{translated(attributes[:name])} in #{translated(candidacy.title)}")
    end
  ...
  context "when edit a component" do
    ...
    let!(:component) do
      create(:component, name: component_name, participatory_space: candidacy)
    end
    ...
    it "updates the component" do
      ...
      visit decidim_admin.root_path
      expect(page).to have_content("updated #{translated(attributes[:name])} in #{translated(candidacy.title)}")
    end
  end
  ...
  context "when remove a component" do
    ...
    let!(:component) do
      create(:component, :unpublished, name: component_name, participatory_space: candidacy)
    end
  ...
  context "when publish and unpublish a component" do
    let!(:component) do
      create(:component, participatory_space: candidacy, published_at:, visible:)
    end
  ...
  def participatory_space_components_path(participatory_space)
    decidim_admin_candidacies.components_path(participatory_space)
  end
end
