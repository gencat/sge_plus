...
describe Decidim::OpenDataExporter do
  ...
  let(:resource_file_name) { "candidacies" }
  let!(:resource) { create(:candidacy, :open, organization:) }
  let(:resource_title) { "## candidacies" }
  let(:help_lines) do
    [
      "reference: The unique reference of the space"
    ]
  end
  let(:unpublished_resource) { create(:candidacy, :created, organization:) }
