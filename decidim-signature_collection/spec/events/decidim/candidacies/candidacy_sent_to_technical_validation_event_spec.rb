...
describe Decidim::Candidacies::CandidacySentToTechnicalValidationEvent do
  include_context "when a simple event"

  let(:resource) { create(:candidacy) }
  let(:participatory_space) { resource }
  let(:event_name) { "decidim.events.candidacies.candidacy_sent_to_technical_validation" }
