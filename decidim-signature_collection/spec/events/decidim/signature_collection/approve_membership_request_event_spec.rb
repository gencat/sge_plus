...
describe Decidim::SignatureCollection::ApproveMembershipRequestEvent do
  include_context "when a simple event"

  let(:user_role) { :affected_user }
  let(:extra) { { author: } }
  let(:event_name) { "decidim.signature_collection.events.approve_membership_request" }
  let!(:resource) { create(:candidacy, :created) }
  ...
  let(:email_intro) { "#{author_nickname} accepted your application to be part of the promoter committee for the candidacy #{resource_title}." }
  let(:email_outro) { "You received this notification because you applied to this candidacy: #{resource_title}" }
  let(:notification_title) { "<a href=\"#{author_profile_url}\">#{author_nickname}</a> accepted your application to be part of the promoter committee for the following candidacy <a href=\"#{resource_url}\">#{resource_title}</a>." }
