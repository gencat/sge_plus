    ...
    describe VoteCandidacy do
      ...
      describe "User votes candidacy" do
        ...
        it "creates a vote" do
          expect do
            command.call
          end.to change(CandidacysVote, :count).by(1)
        end
        ...
        it "notifies the creation" do
          ...
          expect(Decidim::EventsManager)
            .to receive(:publish)
            .with(
              event: "decidim.events.candidacies.candidacy_endorsed",
              event_class: Decidim::Candidacies::EndorseCandidacyEvent,
              resource: candidacy,
              followers: [follower]
            )
        ...
        context "when a new milestone is completed" do
          ...
          it "notifies the followers" do
            ...
            expect(Decidim::EventsManager)
              .to receive(:publish)
              .with(
                event: "decidim.events.candidacies.milestone_completed",
                event_class: Decidim::Candidacies::MilestoneCompletedEvent,
                resource: candidacy,
        ...
        context "when support threshold is reached" do
          ...
          it "notifies the admins" do
            ...
            expect(Decidim::EventsManager)
              .to receive(:publish)
              .with(
                event: "decidim.events.candidacies.support_threshold_reached",
                event_class: Decidim::Candidacies::Admin::SupportThresholdReachedEvent,
                resource: candidacy,
                followers: [admin]
              )
        ...
        context "when candidacy type requires extra user fields" do
          let(:candidacy) do
            create(
              :candidacy,
          ...
          context "when another signature exists with the same hash_id" do
            before do
              create(:candidacy_user_vote, candidacy:, hash_id: form_with_personal_data.hash_id)
            end
          ...
          context "when candidacy type has document number authorization handler" do
            let(:handler_name) { "dummy_authorization_handler" }
            let(:unique_id) { "test_digest" }
            let(:metadata) do
              {
                test: "dummy",
                scope_id: candidacy.scoped_type.scope.id
              }
            end
            ...
            before do
              ...
              candidacy.type.update(document_number_authorization_handler: handler_name)
            end
