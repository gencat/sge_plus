...
module Decidim
  module Candidacies
    describe CandidacysMailer do
      include Decidim::TranslationsHelper

      let(:organization) { create(:organization, host: "1.lvh.me") }
      let(:candidacy) { create(:candidacy, organization:) }
      let(:router) { Decidim::Candidacies::Engine.routes.url_helpers }
      let(:admin_router) { Decidim::Candidacies::AdminEngine.routes.url_helpers }
      ...
      context "when notifies state change" do
        ...
        it "renders the body" do
          expect(mail.body).to include("The candidacy #{decidim_sanitize_translated(candidacy.title)} has changed its status to: #{I18n.t(candidacy.state, scope: "decidim.candidacies.admin_states")}")
        end
      end
