...
module Decidim
  # The data store for a Candidacy in the Decidim::Candidacies component.
  class Candidacy < ApplicationRecord
    ...
    include Decidim::Candidacies::CandidacySlug
    include Decidim::Resourceable
    include Decidim::HasReference
    include Decidim::Randomable
    include Decidim::Searchable
    include Decidim::Candidacies::HasArea
    ...
    belongs_to :scoped_type,
               class_name: "Decidim::CandidacysTypeScope",
               inverse_of: :candidacies

    has_many :votes,
             foreign_key: "decidim_candidacy_id",
             class_name: "Decidim::CandidacysVote",
             dependent: :destroy,
             inverse_of: :candidacy

    has_many :committee_members,
             foreign_key: "decidim_candidacies_id",
             class_name: "Decidim::CandidacysCommitteeMember",
             dependent: :destroy,
             inverse_of: :candidacy
    ...
    # Candidacies are not intended to have categories.
    has_many :categories,
    ...
    scope :with_any_type, lambda { |*original_type_ids|
      ...
      types = CandidacysTypeScope.where(decidim_candidacies_types_id: type_ids).pluck(:id)
      where(scoped_type: types)
    }

    # Redefine the with_any_scope method as the candidacy scope is defined by
    # the candidacy type scope.
    scope :with_any_scope, lambda { |*original_scope_ids|
      ...
      conditions = []
      conditions << "decidim_candidacies_type_scopes.decidim_scopes_id IS NULL" if clean_scope_ids.delete("global")
    ...
    scope :authored_by, lambda { |author|
      co_authoring_candidacy_ids = Decidim::CandidacysCommitteeMember.where(
        decidim_users_id: author
      ).pluck(:decidim_candidacies_id)

      where(
        decidim_author_id: author,
        decidim_author_type: Decidim::UserBaseEntity.name
      ).or(
        where(id: co_authoring_candidacy_ids)
      )
    }
    ...
    searchable_fields({
                      ...
                      index_on_create: ->(_candidacy) { false },
    ...
    def self.export_serializer
      Decidim::Candidacies::DownloadYourDataCandidacySerializer
    end

    def self.log_presenter_class_for(_log)
      Decidim::Candidacies::AdminLog::CandidacyPresenter
    end
    ...
    # Public: Check if an candidacy has been created by an individual person.
    ...
    def signature_period_description
      ...
      case signature_period_type
      when :full_period
        I18n.t("decidim.candidacies.candidacies.show.signature_period.full_period", start: start_str, end: end_str)
      when :from_start_to_indefinite
        I18n.t("decidim.candidacies.candidacies.show.signature_period.from_start_to_indefinite", start: start_str)
      when :from_publication_to_end
        I18n.t("decidim.candidacies.candidacies.show.signature_period.from_publication_to_end", end: end_str)
      when :no_period
        I18n.t("decidim.candidacies.candidacies.show.signature_period.no_period")
      end
    end
    ...
    # Public: Checks if the organization has given an answer for the candidacy.
    #
    # Returns a Boolean.
    def answered?
      answered_at.present?
    ...
    # participatory space or assemblies. For candidacies it will not be directly
    ...
    # For candidacies it will not be directly
    # managed by the user and it will be enabled by default.
    def scopes_enabled
      true
    end

    # Public: Publishes this candidacy
    #
    ...
    def publish!
      ...
      update(
        ...
        signature_end_date: signature_end_date || (Date.current + Decidim::Candidacies.default_signature_time_period_length)
      )
    end

    # Public: Unpublishes this candidacy
    #
    # Returns true if the record was properly saved, false otherwise.
    def unpublish!
    ...
    # Public: Calculates the number of supports required to accept the candidacy for a scope.
    #
    # Returns an Integer.
    def supports_required_for(scope)
      candidacy_type_scopes.find_by(decidim_scopes_id: scope&.id).supports_required
    end
    ...
    def supports_goal_reached?
      votable_candidacy_type_scopes.map(&:scope).all? { |scope| supports_goal_reached_for?(scope) }
    end
    ...
    # Public: Finds all the CandidacyTypeScopes that are eligible to be voted by a user.
    # Usually this is only the `scoped_type` but voting on children of the scoped type is
    # enabled we have to filter all the available scopes in the candidacy type to select
    # the ones that are a descendant of the candidacy type.
    #
    # Returns an Array of Decidim::CandidacysScopeType.
    def votable_candidacy_type_scopes
      return Array(scoped_type) unless type.child_scope_threshold_enabled?

      candidacy_type_scopes.select do |candidacy_type_scope|
        candidacy_type_scope.scope.present? && (scoped_type.global_scope? || scoped_type.scope.ancestor_of?(candidacy_type_scope.scope))
    ...
    def minimum_committee_members
      type.minimum_committee_members || Decidim::Candidacies.minimum_committee_members
    end
    ...
    def shareable_url(share_token)
      EngineRouter.main_proxy(self).candidacy_url(self, share_token: share_token.token)
    end

    def self.ransack(params = {}, options = {})
      Candidacies::CandidacySearch.new(self, params, options)
    end

    private

    # Private: This is just an alias because the naming on CandidacyTypeScope
    # is very confusing. The `scopes` method does not return Decidim::Scope but
    # Decidim::CandidacyTypeScopes.
    #
    # ¯\_(ツ)_/¯
    #
    # Returns an Array of Decidim::CandidacysScopeType.
    def candidacy_type_scopes
      type.scopes
    end

    # Private: A validator that verifies the signature type is allowed by the CandidacyType.
    def signature_type_allowed
      return if published?

      errors.add(:signature_type, :invalid) if type.allowed_signature_types_for_candidacies.exclude?(signature_type)
    end

    def notify_state_change
      return unless saved_change_to_state?

      notifier = Decidim::Candidacies::StatusChangeNotifier.new(candidacy: self)
      notifier.notify
    end
    ...
    ransacker :type_id do
      Arel.sql("decidim_candidacies_type_scopes.decidim_candidacies_types_id")
    end
    ...
    ransacker :id_string do
      Arel.sql(%{cast("decidim_candidacies"."id" as text)})
    end
