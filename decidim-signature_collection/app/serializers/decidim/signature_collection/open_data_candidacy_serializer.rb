...
module Decidim
  module SignatureCollection
    class OpenDataCandidacySerializer < Decidim::Exporters::ParticipatorySpaceSerializer
      # Public: Exports a hash with the serialized data for this candidacy.
      #
      # Note that we do not merge the original serialize method here, as the Candidacy
      # model does not have the same attributes as the other Spaces models.
      def serialize
        {
          reference: resource.reference,
          title: resource.title,
          url: EngineRouter.main_proxy(resource).candidacy_url(resource),
          description: resource.description,
          state: resource.state,
          created_at: resource.created_at,
          updated_at: resource.updated_at,
          published_at: resource.published_at,
          signature_start_date: resource.signature_start_date.presence || resource.type.signature_period_start.to_date,
          signature_end_date: resource.signature_end_date.presence || resource.type.signature_period_end.to_date,
