    ...
    module Admin
      module Filterable
        ...
        included do
          ...
          def base_query
            collection.joins(:scoped_type).left_joins(:area).joins("JOIN decidim_users ON decidim_users.id = decidim_candidacies.decidim_author_id")
          end
          ...
          def filters_with_values
            {
              ...
              type_id_eq: CandidacysType.where(organization: current_organization).pluck(:id),
          ...
          def translated_type_id_eq(id)
            translated_attribute(Decidim::CandidacysType.find_by(id:).title[I18n.locale.to_s])
          end
