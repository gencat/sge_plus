...
module Decidim
  module Candidacies
    ...
    class CandidacysController < Decidim::Candidacies::ApplicationController
      ...
      include Decidim::Candidacies::Orderable
      ...
      def show
        ...
        if current_candidacy.type.published?
          ...
          flash[:alert] = I18n.t("decidim.candidacies.show.type_not_published")
      ...
      def edit
        ...
        @form = form(Decidim::Candidacies::CandidacyForm)
                .from_model(
                  current_candidacy,
      ...
      def update
        ...
        UpdateCandidacy.call(current_candidacy, @form) do
          on(:ok) do |candidacy|
            flash[:notice] = I18n.t("success", scope: "decidim.candidacies.update")
            redirect_to candidacy_path(candidacy)
          end

          on(:invalid) do
            flash.now[:alert] = I18n.t("error", scope: "decidim.candidacies.update")
      ...
      def print
        ...
        output = Decidim::Candidacies::ApplicationFormPDF.new(current_candidacy).render
      ...
      def search_collection
        Candidacy
          ...
          .joins("JOIN decidim_users ON decidim_users.id = decidim_candidacies.decidim_author_id")
          .joins(scoped_type: :type)
          .where(organization: current_organization)
          .where(decidim_candidacies_types: { published: true })
      end
      ...
      def set_candidacies_settings
        @candidacies_settings ||= Decidim::CandidacysSettings.find_by(organization: current_organization)
      end
    end
  end
end
