...
module Decidim
  module Candidacies
    class CandidacySignaturesController < Decidim::Candidacies::ApplicationController
      ...
      def create
        ...
        @form = form(Decidim::Candidacies::VoteForm)
                .from_params(
      ...
      def store_personal_data
        ...
        if @vote_form.invalid?
          flash[:alert] = I18n.t("personal_data.invalid", scope: "decidim.candidacies.candidacy_votes")
          @form = @vote_form
      ...
      def store_sms_phone_number
        ...
        ValidateMobilePhone.call(@form, current_user) do
          ...
          on(:invalid) do
            flash[:alert] = I18n.t("sms_phone.invalid", scope: "decidim.candidacies.candidacy_votes")
      ...
      def store_sms_code
        ...
        ValidateSmsCode.call(@form, session_sms_code) do
          ...
          on(:invalid) do
            flash[:alert] = I18n.t("sms_code.invalid", scope: "decidim.candidacies.candidacy_votes")
            render :sms_code
          end
        end
      end
      ...
      def finish
        ...
        VoteCandidacy.call(@vote_form) do
          ...
          on(:invalid) do |vote|
            ...
            flash[:alert] = I18n.t("create.invalid", scope: "decidim.candidacies.candidacy_votes")
      ...
      def build_vote_form(parameters)
        @vote_form = form(Decidim::Candidacies::VoteForm).from_params(parameters).tap do |form|
      ...
      def session_vote_form
        ...
        @vote_form = form(Decidim::Candidacies::VoteForm).from_params(attributes)
      end

      def check_session_personal_data
        return if session[:candidacy_vote_form].present? && session_vote_form&.valid?

        flash[:alert] = I18n.t("create.error", scope: "decidim.candidacies.candidacy_votes")
