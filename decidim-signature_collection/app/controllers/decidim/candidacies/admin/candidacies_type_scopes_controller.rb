...
module Decidim
  module Candidacies
    module Admin
      ...
      class CandidacysTypeScopesController < Decidim::Candidacies::Admin::ApplicationController
        ...
        def create
          ...
          CreateCandidacyTypeScope.call(@form) do
            on(:ok) do |candidacy_type_scope|
              flash[:notice] = I18n.t("decidim.candidacies.admin.candidacies_type_scopes.create.success")
              redirect_to edit_candidacies_type_path(candidacy_type_scope.type)
            end

            on(:invalid) do
              flash.now[:alert] = I18n.t("decidim.candidacies.admin.candidacies_type_scopes.create.error")
              render :new
            end
          end
        end
        ...
        def update
          ...
          UpdateCandidacyTypeScope.call(@form, current_candidacy_type_scope) do
            on(:ok) do
              flash[:notice] = I18n.t("decidim.candidacies.admin.candidacies_type_scopes.update.success")
              redirect_to edit_candidacies_type_path(resource.type)
            end

            on(:invalid) do
              flash.now[:alert] = I18n.t("decidim.candidacies.admin.candidacies_type_scopes.update.error")
              render :edit
            end
          end
        end
        ...
        def destroy
          ...
          redirect_to edit_candidacies_type_path(current_candidacy_type_scope.type), flash: {
            notice: I18n.t("decidim.candidacies.admin.candidacies_type_scopes.destroy.success")
          }
        end
        ...
        def current_candidacy_type_scope
          @current_candidacy_type_scope ||= CandidacysTypeScope.joins(:type).where(decidim_candidacies_types: { organization: current_organization }).find(params[:id])
        end

        def current_candidacy_type
          @current_candidacy_type ||= CandidacysType.find(params[:candidacies_type_id])
        end

        def candidacy_type_scope_form
          form(Decidim::Candidacies::Admin::CandidacyTypeScopeForm)
        end
      end
    end
  end
