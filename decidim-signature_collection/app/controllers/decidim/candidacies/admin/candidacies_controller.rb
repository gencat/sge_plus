...
module Decidim
  module Candidacies
    module Admin
      ...
      class CandidacysController < Decidim::Candidacies::Admin::ApplicationController
        include Decidim::Candidacies::NeedsCandidacy
        include Decidim::Candidacies::SingleCandidacyType
        include Decidim::Candidacies::TypeSelectorOptions
        include Decidim::Candidacies::Admin::Filterable
        include Decidim::Admin::ParticipatorySpaceAdminBreadcrumb

        helper ::Decidim::Admin::ResourcePermissionsHelper
        helper Decidim::Candidacies::CandidacyHelper
        helper Decidim::Candidacies::SignatureTypeOptionsHelper
        ...
        def edit
          ...
          @form = form(Decidim::Candidacies::Admin::CandidacyForm)
                  .from_model(
                    current_candidacy,
        ...
        def update
          ...
          Decidim::Candidacies::Admin::UpdateCandidacy.call(@form, current_candidacy) do
            on(:ok) do |candidacy|
            ...
            on(:invalid) do
              flash.now[:alert] = I18n.t("candidacies.update.error", scope: "decidim.candidacies.admin")
              render :edit, layout: "decidim/admin/candidacy"
            end
          end
        end
        ...
        def publish
          ...
          PublishCandidacy.call(current_candidacy, current_user) do
            on(:ok) do
              flash[:notice] = I18n.t("candidacies.publish.success", scope: "decidim.candidacies.admin")
        ...
        def unpublish
          ...
          UnpublishCandidacy.call(current_candidacy, current_user) do
            on(:ok) do
              flash[:notice] = I18n.t("candidacies.unpublish.success", scope: "decidim.candidacies.admin")
              redirect_to decidim_admin_candidacies.edit_candidacy_path(current_candidacy)
            end
          end
        end
        ...
        def discard
          ...
          DiscardCandidacy.call(current_candidacy, current_user) do
            on(:ok) do
              flash[:notice] = I18n.t("candidacies.discard.success", scope: "decidim.candidacies.admin")
              redirect_to decidim_admin_candidacies.edit_candidacy_path(current_candidacy)
            end
          end
        end
        ...
        def accept
          ...
          AcceptCandidacy.call(current_candidacy, current_user) do
            on(:ok) do
              flash[:notice] = I18n.t("candidacies.accept.success", scope: "decidim.candidacies.admin")
              redirect_to decidim_admin_candidacies.edit_candidacy_path(current_candidacy)
            end
          end
        end
        ...
        def reject
          ...
          RejectCandidacy.call(current_candidacy, current_user) do
            on(:ok) do
              flash[:notice] = I18n.t("candidacies.reject.success", scope: "decidim.candidacies.admin")
              redirect_to decidim_admin_candidacies.edit_candidacy_path(current_candidacy)
            end
          end
        end
          ...
          SendCandidacyToTechnicalValidation.call(current_candidacy, current_user) do
            on(:ok) do
              redirect_to EngineRouter.main_proxy(current_candidacy).candidacies_path(candidacy_slug: nil), flash: {
                notice: I18n.t(
                  "success",
                  scope: "decidim.candidacies.admin.candidacies.edit"
                )
              }
            end
          end
        ...
        def export
          ...
          Decidim::Candidacies::ExportCandidacysJob.perform_later(
            current_user,
        ...
        def show_candidacy_type_callout?
          Decidim::CandidacysType.where(organization: current_organization).none?
        end

        def collection
          @collection ||= ManageableCandidacys.for(current_user)
        end
