  ...
  module SignatureCollection
    ...
    class CreateCandidacyController < Decidim::SignatureCollection::ApplicationController
      ...
      def select_candidacy_type
        @form = form(Decidim::SignatureCollection::SelectCandidacyTypeForm).from_params(params)
      ...
      def fill_data
        @form = if session[:candidacy_id].present?
                  form(Decidim::SignatureCollection::CandidacyForm).from_model(current_candidacy, { candidacy_type: })
                else
                  extras = { type_id: candidacy_type_id, signature_type: candidacy_type.signature_type }
                  form(Decidim::SignatureCollection::CandidacyForm).from_params(params.merge(extras), { candidacy_type: })
                end
      end

      def store_data
        @form = form(Decidim::SignatureCollection::CandidacyForm).from_params(params, { candidacy_type: })
      ...
      def promotal_committee_required?
        ...
        minimum_committee_members = candidacy_type.minimum_committee_members ||
                                    Decidim::SignatureCollection.minimum_committee_members
