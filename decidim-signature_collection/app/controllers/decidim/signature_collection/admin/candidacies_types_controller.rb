...
module Decidim
  module SignatureCollection
    module Admin
      ...
      # organization.
      class CandidaciesTypesController < Decidim::SignatureCollection::Admin::ApplicationController
        ...
        def create
          ...
          CreateCandidacyType.call(@form) do
            on(:ok) do |candidacy_type|
              flash[:notice] = I18n.t("decidim.signature_collection.admin.candidacies_types.create.success")
              redirect_to edit_candidacies_type_path(candidacy_type)
            end

            on(:invalid) do
              flash.now[:alert] = I18n.t("decidim.signature_collection.admin.candidacies_types.create.error")
              render :new
            end
          end
        end
        ...
        def update
          ...
          UpdateCandidacyType.call(@form, current_candidacy_type) do
            on(:ok) do
              flash[:notice] = I18n.t("decidim.signature_collection.admin.candidacies_types.update.success")
              redirect_to edit_candidacies_type_path(current_candidacy_type)
            end

            on(:invalid) do
              flash.now[:alert] = I18n.t("decidim.signature_collection.admin.candidacies_types.update.error")
              render :edit
            end
          end
        end
        ...
        def destroy
          ...
          redirect_to candidacies_types_path, flash: {
            notice: I18n.t("decidim.signature_collection.admin.candidacies_types.destroy.success")
          }
        end
        ...
        def current_candidacy_type
          @current_candidacy_type ||= CandidaciesType.where(organization: current_organization).find(params[:id])
        end

        def candidacy_type_form
          form(Decidim::SignatureCollection::Admin::CandidacyTypeForm)
        end
      end
    end
  end
