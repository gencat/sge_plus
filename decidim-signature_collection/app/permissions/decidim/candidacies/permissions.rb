  ...
  module Candidacies
    class Permissions < Decidim::DefaultPermissions
      def permissions
        ...
        return Decidim::Candidacies::Admin::Permissions.new(user, permission_action, context).permissions if permission_action.scope == :admin

        return permission_action if candidacy && !candidacy.is_a?(Decidim::Candidacy)
      ...
      def creation_enabled?
        Decidim::Candidacies.creation_enabled && (
        Decidim::Candidacies.do_not_require_authorization ||
      ...
      def access_request_without_user?
        (!candidacy.open? && candidacy.promoting_committee_enabled?) || Decidim::Candidacies.do_not_require_authorization
      end
      ...
      def print_candidacy?
        ...
        toggle_allow(Decidim::Candidacies.print_enabled && (authorship_or_admin? || committee_member?))
      end

      def committee_member?
        CandidacysPromoted.by(user).exists?(id: candidacy.id)
      end
