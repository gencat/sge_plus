  ...
  module Candidacies
    ...
    class CandidacyForm < Form
      ...
      validates :signature_end_date, date: { after: Date.current }, if: lambda { |form|
        form.context.candidacy_type.custom_signature_end_date_enabled? && form.signature_end_date.present?
      }
      ...
      def map_model(model)
        ...
        self.signature_type = model.signature_type || candidacy_type.signature_type
        self.title = translated_attribute(model.title)
        self.description = translated_attribute(model.description)
      end

      def type
        @type ||= type_id ? Decidim::CandidacysType.find(type_id) : context.candidacy.type
      end
      ...
      def candidacy_type
        @candidacy_type ||= type_id ? CandidacysType.find(type_id) : context.candidacy.type
      end
      ...
      def scope_exists
        ...
        errors.add(:scope_id, :invalid) unless CandidacysTypeScope.exists?(type:, scope:)
      end
