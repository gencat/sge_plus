...
* A CRUD engine to manage candidacies.

* Public views for candidacies via a high level section in the main menu.
...
This is the case if you want to enable the creation of candidacies even when no authorization method is set.
...
This task retrieves all published candidacies whose support method is online and the support
period has expired. Candidacies that have reached the minimum supports required will pass
to state __accepted__. The candidacies without enough supports will pass to __rejected__ state.

Candidacies with offline support method enabled (pure offline or mixed) will get its status updated
...
* Title and description will be hashed using the same locale used by the candidacy author. In case there is more
