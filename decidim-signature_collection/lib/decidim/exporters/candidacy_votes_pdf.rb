...
module Decidim
  module Exporters
    ...
    #
    class CandidacyVotesPDF < PDF
      def initialize(collection, candidacy, serializer = Serializer)
        @candidacy = candidacy
      ...
      def add_data!
        composer.formatted_text([translated_attribute(candidacy.title)], style: :candidacy_title)
      ...
      def styles
        {
          candidacy_title: { font: bold_font, text_align: :center, font_size: 12, margin: [0, 0, 10, 0] },
          candidacy_th: { font: bold_font, font_size: 10 },
          candidacy_td: { font:, font_size: 10 },
          vote_th: { font: bold_font, font_size: 11 },
          vote_td: { font:, font_size: 10 }
        }
      end

      def add_candidacy_data
        data_header = [
          layout.text(I18n.t("models.candidacies_votes.fields.candidacy_id", scope: "decidim.admin"), style: :candidacy_th),
          layout.text(I18n.t("models.candidacies_votes.fields.candidacy_title", scope: "decidim.admin"), style: :candidacy_th),
