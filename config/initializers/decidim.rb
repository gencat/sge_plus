...
Decidim.configure do |config|
  ...
  config.available_locales = Rails.application.secrets.decidim[:available_locales].presence || [:ca, :es, :oc]
...
if Decidim.module_installed? :signature_collection
  Decidim::SignatureCollection.configure do |config|
    unless Rails.application.secrets.dig(:decidim, :signature_collection, :creation_enabled) == "auto"
    ...
    config.stats_cache_expiration_time = Rails.application.secrets.dig(:decidim, :signature_collection, :stats_cache_expiration_time).to_i.minutes
    config.max_time_in_validating_state = Rails.application.secrets.dig(:decidim, :signature_collection, :max_time_in_validating_state).to_i.days
    unless Rails.application.secrets.dig(:decidim, :signature_collection, :print_enabled) == "auto"
      config.print_enabled = Rails.application.secrets.dig(:decidim, :signature_collection, :print_enabled).present?
    end
    config.do_not_require_authorization = Rails.application.secrets.dig(:decidim, :signature_collection, :do_not_require_authorization).present?
  end
end
